...
def convert_plsql_to_database(plsql_code, db_type, HUGGINGFACE_API_TOKEN):
    try:
        ...
        # Display stepwise info (only at the beginning)
...
def process_plsql_zip(file, db_type, HUGGINGFACE_API_TOKEN):
    ...
    # Display overall steps once
    st.info("Step 1: Splitting the code into chunks... 💬")
    st.info("Step 2: Generating embeddings... 📊")
    st.info("Step 3: Loading the language model... 🚀")
    st.info(f"Step 4: Converting PL/SQL to {db_type}... 🔄")

    # Clear previous conversion steps messages
    step_messages = [
        st.empty(),  # for Step 1 message
        st.empty(),  # for Step 2 message
        st.empty(),  # for Step 3 message
        st.empty(),  # for Step 4 message
    ]
        ...
        if plsql_file_path.endswith(".sql"):
            try:
                with open(plsql_file_path, 'r') as f:
                    ...
                    # Clear the previous messages and show current file conversion progress
                    step_messages[0].empty()
                    step_messages[1].empty()
                    step_messages[2].empty()
                    step_messages[3].empty()
